...
async fn main() -> std::io::Result<()> {
    ...
    log_info!("HTTP start");
    HttpServer::new(move || {
        // 统一鉴权中间件配置：启动时从 GLOBAL_CONFIG 取快照
        // individual 模式下无 master 字段（无回源鉴权，仅 uuid 旧链路）
        ...
        App::new()
            ...
            // 统一用户鉴权：Bearer 回源 watchman 鉴权 + uuid 旧链路放行
            .wrap(UserAuth::new(user_auth_config))
