...
- **Crate-specific atomic operations**: if an atomic operation is used by only one crate with no prospect of reuse, it belongs to that crate's `infra/` module (e.g. the filesystem primitives in `file-agent/src/infra/file_op.rs`, the log-report outbound HTTP in `jc-worker/src/infra/log_update.rs`, and the MQ consume loop in `jc-worker/src/infra/mq_consumer.rs`).
...
- **Scheduler**: `jc-commander`'s `util/scheduler.rs` (a timing-wheel scheduler) is positioned as a standalone scheduler component that belongs to none of the three layers. It stays in `util/` rather than `infra/` precisely because it is a cross-cutting component, not an atomic operation. Its constraints match the orchestration layer's: DB calls must go through `model/`, and MQ calls must go through the `share_lib::infrastructure::mq_client` atomic module — inlining diesel or lapin code inside the scheduler is forbidden.
...
- **jc-worker**: a pure MQ consumer with no `api/`, `middleware/`, or `model/`. Its config layer is `config/worker.rs` (`jc-worker/src/config/worker.rs`); the startup chain switches to `#[tokio::main]` plus invoking the consume loop in `infra/mq_consumer.rs` (`jc-worker/src/main.rs`); business orchestration lives in `service/task.rs`, and crate-private atomic operations are consolidated in `infra/` (the MQ consume loop `mq_consumer.rs` and the log-report outbound HTTP `log_update.rs`).
- **file-agent**: no database — `mod model;` is commented out as a whole in `main.rs` and there is no r2d2 pool; multipart upload logic is in `service/file_manage.rs`, and filesystem primitives are consolidated in `infra/file_op.rs`.
- **yell**: the business layer uses the singular directory `service/` (the former plural `services/` has been eliminated) and carries orchestration logic only; `service/channel.rs` defines the `Channel` trait that every channel must implement (`channel_type` / `preflight` / `render` / `send`) plus the unified delivery skeleton `deliver_template` (with `create_record` / `update_record` / `resolve_config`), and `NotificationRouter` in `service/notification_router.rs` registers and dispatches all channels. Channel call-out implementations (atomic operations) have been moved into `infra/`, subdivided per channel (`bark/`, `gotify/`, `mail/`, `teams/`, `teams_hook/`, `webhook/`), keeping only the "validate / call out" responsibility.
...
- `/api/hey` is registered for both GET and POST in watchman-backend and yell, but only for POST in the other crates (e.g. `file-agent/src/config/app.rs:11`); callers should use POST for compatibility.
...
- The `refresh_master` route is unified as `/api/manage/refresh_master` across crates (cloud-api / jc-commander migrated from `/api/refresh_master`); each crate's `authenticate_bypass` whitelist matches this route. This route and its sibling `register_help` (GET, auth-required, returns a pre-filled registration JSON) are cfg-gated out in `individual` mode.
