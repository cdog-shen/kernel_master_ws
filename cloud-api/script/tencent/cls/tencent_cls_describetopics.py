...
import sys
import os
from package_import import ApiClient_CLS
...
try:
    ...
    client = ApiClient_CLS(AK, SK, region, endpoint)

    req = client.ModelsHandler().DescribeTopicsRequest()
    req.from_json_string(jsonStr=json.dumps(params))

    resp = client.ClientHandler().DescribeTopics(req)
