...
use crate::model::schema::notification_groups::{self, dsl::*};
...
pub struct NotificationGroupModel {
    pub id: i32,
    pub name: String,
...
impl NotificationGroupModel {
    /// get model full data
    pub fn get_model_info_with_filter(
        filter: &Map<String, Value>,
        conn: &mut PgConnection,
    ) -> Result<Vec<Value>, (u8, String)> {
        let mut query = notification_groups
            .into_boxed()
            .select(NotificationGroupModel::as_select());
        ...
        match query.get_results::<NotificationGroupModel>(conn) {
            Ok(vec_item_info) => {
...
impl NotificationGroupModel {
    pub fn new(
        info: &NotificationGroupInfo,
        conn: &mut PgConnection,
    ) -> Result<usize, (u8, String)> {
        match diesel::insert_into(notification_groups)
            .values(info)
            .execute(conn)
        {
    ...
    pub fn update(
        ...
        match diesel::update(notification_groups.filter(id.eq(info.id.unwrap())))
            .set(info)
            .execute(conn)
        {
    ...
    pub fn delete(_id: i32, conn: &mut PgConnection) -> Result<usize, (u8, String)> {
        match diesel::delete(notification_groups.filter(id.eq(_id))).execute(conn) {
            Ok(num_of_eff) => match num_of_eff {
